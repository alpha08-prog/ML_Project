...
/* ── Types ────────────────────────────────────── */
interface EEGPoint       { time: number; amplitude: number }
interface UMAPPoint      { x: number; y: number; type: 'real' | 'synthetic' | 'majority' }
interface ChannelData    { channels: string[]; real: number[]; synthetic: number[]; importance: number[] }
...
export default function DataVisualization() {
  ...
  const fetchEEG = useCallback(async () => {
    ...
    try {
      ...
      setEegData(data.time.map((t: number, i: number) => ({ time: t, amplitude: data.amplitude[i] })))
    } catch {
          ...
          <div className="card hover-float glass">
            <div className="chart-header">
              ...
              <div className="channel-selector">
                <label>Channel:</label>
                <select className="channel-select" value={selectedChannel}
                  onChange={(e) => setSelectedChannel(Number(e.target.value))}>
                  {CHANNEL_NAMES.map((name, idx) => <option key={idx} value={idx}>{name}</option>)}
                </select>
              </div>
            </div>
            {error && <p style={{ color: 'var(--warning)', marginBottom: 'var(--spacing-md)', fontSize: '0.85rem' }}>{error}</p>}
              ...
              <LineChart>
                ...
                {CHANNEL_NAMES.slice(0, 8).map((name, idx) => (
                  <Line
                    key={idx}
                    type="monotone"
                    data={generateDemoSignal(128, idx).map((d) => ({ ...d, [`ch${idx}`]: d.amplitude }))}
                    dataKey={`ch${idx}`}
        ...
        <div className="visualization-section">
          <div className="card hover-float glass">
            ...
            <p className="chart-description">
              Dimensionality reduction showing real minority samples, GAN-generated synthetic samples,
              and majority class samples in 2D space.
            </p>
            {loading && <div className="loading-container" style={{ minHeight: 200 }}><p>Loading…</p></div>}
            {error   && <div style={{ color: 'var(--error)', marginBottom: 'var(--spacing-md)' }}>{error}</div>}
            {!loading && !error && (
              <ResponsiveContainer width="100%" height={500}>
                <ScatterChart>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey="x" label={{ value: 'UMAP 1', position: 'insideBottom', offset: -5 }} />
                  <YAxis dataKey="y" label={{ value: 'UMAP 2', angle: -90, position: 'insideLeft' }} />
        ...
        <div className="visualization-section">
          <div className="card hover-float glass">
            ...
            {!loading && !error && channelData && (
              <ResponsiveContainer width="100%" height={400}>
                <LineChart data={channelData.channels.map((ch, i) => ({
                  channel: ch, real: channelData.real[i], synthetic: channelData.synthetic[i],
                }))}>
                  <CartesianGrid strokeDasharray="3 3" />
                  ...
                  <Tooltip />
                  <Legend />
                  <Line type="monotone" dataKey="real"      stroke="#10b981" name="Real"      strokeWidth={2} />
                  <Line type="monotone" dataKey="synthetic" stroke="#6366f1" name="Synthetic" strokeWidth={2} />
                </LineChart>
              </ResponsiveContainer>
            )}
          </div>
          ...
          {channelData && (
            <div className="card hover-float glass">
              ...
              <p className="chart-description">
                Kolmogorov-Smirnov statistics per channel — real vs synthetic distribution similarity.
              </p>
